...
if (count($errors) == 0) {
    $id_query = "select max(id) as max from items ";
    $row = mysqli_query($db,$id_query);
    if($resault = mysqli_fetch_assoc($row)){
    $id = ++$resault['max'];}
    else
    {$id = 1;}
      $query="insert into items(id,itemname,price,amount,exp_d,type,notes)Values('$id','$itemname','$price','$amount','$exp_d','$type','$notes')";
        mysqli_query($db,$query);
